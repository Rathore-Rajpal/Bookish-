...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_FILES["photo"]["name"])) {
        ...
        // Handle photo upload
        $targetDir = "uploads/"; // Updated to use 'image' directory
    ...
    if (!empty($itemName) && !empty($category) && !empty($description) && !empty($price) && !empty($photoName) && !empty($quantity)) {
        ...
        $sqlBuy = "INSERT INTO buy_items (item_name, category, description, price, photo, qty) VALUES ('$itemName', '$category', '$description', '$price', '$photoName', '$quantity')";

        if ($conn->query($sqlBuy) === TRUE) {
            // Insert into rent_items
            $sqlRent = "INSERT INTO rent_items (item_name, category, description, price, photo, qty) VALUES ('$itemName', '$category', '$description', '$price', '$photoName', '$quantity')";

            if ($conn->query($sqlRent) === TRUE) {
                echo "<script>alert('Book added successfully to both buy and rent items.');</script>";
            } else {
                echo "Error inserting into rent_items: " . $conn->error;
            }
        } else {
            echo "Error: " . $sqlBuy . "<br>" . $conn->error;
        }
    }
